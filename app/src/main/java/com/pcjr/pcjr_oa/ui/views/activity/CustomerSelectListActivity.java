 ...
 */
public class CustomerSelectListActivity extends BaseSwipeRefreshActivity implements CustomerContactRelationView {
    ...
    protected void initListeners() {
        ...
        adapter.setOnItemClickListener((a, view, position) -> {
            ...
            if (current.getIsSelected()) {
                ...
                relation.setId("0");
                relation.setCustomerId(current.getId());
    ...
    public void onRefresh() {
        super.onRefresh();
        presenter.getCustomerList(page, query, customerContact.getId(), customerContact.getType());
    }
    ...
    public void onGetRelationshipListSuccess(List<CustomerContactRelation> data) {
        if (data.size() > 0) {
            for (CustomerContactRelation relation : data) {
                map.put(relation.getCustomerId(), relation);
            }
        }
        txtCheckResult.setText(Html.fromHtml("已选择 <font color='#f54246'>" + map.size() + "</font> 项"));
    }
